...
class MilvusStorage:
    def create_schema(self):
        schema_fields = [
            ...
            FieldSchema(name='document_id', dtype=DataType.VARCHAR, description='Document Id', max_length=256),
            ...
            # Content
            FieldSchema(name="chunk_text", dtype=DataType.VARCHAR, description='Chunk original text', max_length=65535),
            FieldSchema(name="chunk_text_vector", dtype=DataType.FLOAT_VECTOR, description='Chunk embedding vector', dim=384),  # Text embedding dim
            # Version control
...
class MilvusClient:
    ...
    def insert_nodes(
        ...
        for node in nodes:
            metadata = node if isinstance(node, dict) else node.metadata
